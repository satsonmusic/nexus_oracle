...
export default function WarRoomPage() {
  ...
  // Scroll to answer panel when response arrives
  useEffect(() => {
    if (quickAnswer && status === "complete") {
      setTimeout(() => {
        const el = document.getElementById("answer-panel");
  ...
  const classifyPrompt = useCallback(async (t: string): Promise<"SOVEREIGN"|"CODE"|"QUICK"> => {
    try {
      const r = await fetch(`${API_BASE}/nexus/classify`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ task: t })
      });
      const d = await r.json();
      const mode = (d.mode || "SOVEREIGN").toUpperCase();
  ...
  const handleLaunch = useCallback(async () => {
    ...
    if (detectedMode !== "SOVEREIGN") {
      try {
        ...
        // Priority 1: respond_to_user tool — this is the clean final answer
        const respondStep = results.find((s: any) =>
          s.tool === "respond_to_user" && s.status === "success" && s.output
        );

        // Priority 2: any successful non-search tool output that has real content
        const usefulSteps = results.filter((s: any) => {
          if (s.tool === "respond_to_user") return false;
          if (s.status !== "success") return false;
          if (!s.output) return false;
          const out = String(s.output).toLowerCase();
          // Filter out empty/error search results
          if (out.includes("no internet results") || out.includes("no results found")) return false;
          if (out.length < 20) return false;
          return true;
        });

        let answer = "";
        if (respondStep?.output) {
          answer = String(respondStep.output);
        } else if (usefulSteps.length > 0) {
          answer = usefulSteps.map((s: any) => String(s.output || "")).join("\n\n");
        } else {
          // Last resort: clean up thoughts by removing internal planning language
          const raw = d.plan?.thoughts || "No response generated.";
          // Strip lines that look like internal task descriptions
          answer = raw.split("\n")
            .filter((line: string) => {
              const l = line.toLowerCase();
              return !l.startsWith("the task is") &&
                     !l.startsWith("there's a need") &&
                     !l.startsWith("there is a need") &&
                     !l.includes("cross-reference") &&
                     l.trim().length > 0;
            })
            .join("\n")
            .trim();
        }
        setQuickAnswer(answer.trim() || "No response generated.");
      } catch (e) {
  ...
  return (
    <div style={{
      ...
      <div style={{ position: "relative", zIndex: 1, padding: "1.5rem 2rem" }}>
        ...
        {quickAnswer && (
          <div id="answer-panel" style={{ marginBottom: "1.5rem", padding: "1rem",
            background: `${modeColor[mode]}06`,
            border: `1px solid ${modeColor[mode]}44`,
            borderLeft: `3px solid ${modeColor[mode]}`,
            borderRadius: 2,
          }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: "0.75rem" }}>
              <span style={{ fontSize: "1rem" }}>{mode === "CODE" ? "⌨" : "⚡"}</span>
              <span style={{ fontFamily: "'Orbitron',sans-serif", fontSize: "0.65rem", fontWeight: 600,
                color: modeColor[mode], letterSpacing: "0.2em" }}>
                {mode === "CODE" ? "CODE OUTPUT" : mode === "QUICK" ? "QUICK ANSWER" : "DIRECT RESPONSE"}
              </span>
            </div>
            <div style={{ whiteSpace: "pre-wrap", fontSize: "0.85rem",
              color: "rgba(224,240,255,0.9)", fontFamily: "'Share Tech Mono',monospace",
              lineHeight: 1.7, borderTop: `1px solid ${modeColor[mode]}22`, paddingTop: "0.75rem" }}>
              {quickAnswer}
            </div>
          </div>
        )}
